...
/// Extracts visible transcript text for bounded history retrieval, excluding private reasoning and prompts.
pub fn transcript_item_text(item: &Value) -> Option<(&'static str, String)> {
...
mod tests {
    ...
    fn history_preserves_ordered_image_references_even_on_internal_materializations() {
        ...
        let (_, text) = transcript_item_text(&result).expect("tool history");
        ...
        materialization["content"]
            ...
            .expect("content")
            .push(image);
        let (_, text) = transcript_item_text(&materialization).expect("image history");
        assert!(text.contains("screen-id"));
