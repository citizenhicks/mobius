...
xcrun swift -e 'import AppKit; for name in ["MobiusLogo", "MobiusMenuBar"] { precondition(Bundle(path: CommandLine.arguments[1])?.image(forResource: name) != nil, "Packaged \(name) is missing") }' "$app"
