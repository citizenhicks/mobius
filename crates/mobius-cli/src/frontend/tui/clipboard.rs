...
use mobius::middleware::session_files::MAX_UPLOAD_CHUNK_BYTES;
...
const MAX_SESSION_BYTES: u64 = 250 * 1024 * 1024;

pub(super) fn read_clipboard(
    existing: &[SessionFileReference],
) -> Result<Vec<UploadCandidate>, String> {
    let remaining = MAX_ATTACHMENT_COUNT.saturating_sub(existing.len());
    if remaining == 0 {
        return Err(format!(
    ...
    if !files.is_empty() {
        return file_candidates(files, existing, remaining);
    }

    let image = clipboard
        .get_image()
        .map_err(|error| format!("clipboard has no files or image: {error}"))?;
    let candidate = bitmap_candidate(image.width, image.height, image.bytes.as_ref())?;
    validate_total_size(existing, std::slice::from_ref(&candidate))?;
    Ok(vec![candidate])
}
...
fn file_candidates(
    ...
    if paths.len() > remaining {
        return Err(format!(
            "pasting {} files would exceed the {MAX_ATTACHMENT_COUNT}-attachment message limit",
            paths.len()
        ));
    }
    let candidates = paths
        .into_iter()
        .map(UploadCandidate::from_file)
        .collect::<Result<Vec<_>, _>>()?;
    validate_total_size(existing, &candidates)?;
    Ok(candidates)
}
...
fn validate_total_size(
    ...
    if total > MAX_SESSION_BYTES {
        return Err(format!(
            "pasted attachments exceed the {MAX_SESSION_BYTES}-byte session limit"
        ));
    }
    Ok(())
}
...
fn validate_size(size: u64) -> Result<(), String> {
    if !(1..=MAX_FILE_BYTES).contains(&size) {
        return Err(format!("attachment size must be 1–{MAX_FILE_BYTES} bytes"));
    }
    Ok(())
}
...
impl ClipboardUploads {
    ...
    async fn next_transfer(
        ...
        if !(1..=MAX_UPLOAD_CHUNK_BYTES).contains(&max_chunk_bytes) {
...
mod tests {
    ...
    async fn copied_files_are_validated_and_preserved() {
        ...
        let candidates = file_candidates(vec![first, second], &[], MAX_ATTACHMENT_COUNT)
    ...
    fn local_attachment_bounds_reject_unsafe_inputs() {
        ...
        assert!(file_candidates(vec![PathBuf::new(), PathBuf::new()], &[], 1).is_err());
    }
