...
final class SwarmStatsTests: XCTestCase {
    ...
    func testCollapsedMarkdownPreviewRendersMentionEmphasisWithoutSourceMarkers() {
        let preview = inlineMarkdownPreview("**@basil** please review")

        XCTAssertEqual(String(preview.characters), "@basil please review")
        XCTAssertTrue(preview.runs.contains {
    ...
    func testCollapsedMarkdownPreviewPreservesFencedCodeLineBreaks() {
        let source = "```\nline one\nline two\n```"
