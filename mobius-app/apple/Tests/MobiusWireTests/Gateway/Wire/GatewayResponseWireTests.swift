...
extension GatewayWireTests {
    func testJSONValuePreservesExactIntegerNumbers() throws {
        let value = try decoder().decode(
            JSONValue.self,
            from: Data(
                #"{"large":9007199254740993,"maximum":18446744073709551615,"minimum":-9223372036854775808,"fraction":0.5,"boolean":true}"#
                    .utf8)
        )
    ...
    func testPeerAuthorPreservesItsSemanticSymbol() throws {
        let author = try decoder().decode(
            MessageAuthor.self,
            from: Data(
                #"{"type":"peer","message_id":"handoff","session_id":"voice-session","handle":"voice agent","symbol":"voice"}"#
                    .utf8)
        )
        ...
        XCTAssertThrowsError(
            try decoder().decode(
                MessageAuthor.self,
                from: Data(
                    #"{"type":"peer","message_id":"handoff","session_id":"voice-session","handle":"voice agent","symbol":42}"#
                        .utf8)
            ))
    }
    ...
    func testV28MessageTargetsAreStrict() throws {
        let operation = try decoder().decode(
            AgentOperation.self,
            from: Data(
                #"{"type":"capability_command","capability":"sessions","command":"fork","arguments":"","input":null,"target":{"checkpoint_sequence":12,"batch_item_count":3}}"#
                    .utf8
            )
        )
    ...
    func testMessageEventDecodesTypedAuthorAndActualDelivery() throws {
        ...
        XCTAssertEqual(
            message.author,
            .peer(
                messageID: "message-1", sessionID: "chat-reviewer", handle: "@reviewer", symbol: nil
            )
        )
    ...
    func testSessionFileResponsesMatchV28() throws {
        ...
        let started = try decodeEnvelope(
            #"{"version":27,"type":"session_file_upload_ready","request_id":"begin-1","session_id":"chat-1","upload_id":"upload-1","max_chunk_bytes":262144}"#
        )
        guard
            case .sessionFileUploadReady(let requestID, let sessionID, let uploadID, let limit) =
                started
        else {
        ...
        let accepted = try decodeEnvelope(
            #"{"version":27,"type":"session_file_upload_chunk_accepted","request_id":"chunk-1","session_id":"chat-1","upload_id":"upload-1","next_offset":3}"#
        )
        ...
        let uploaded = try decodeEnvelope(
            #"{"version":27,"type":"session_file_upload_completed","request_id":"finish-1","session_id":"chat-1","file":\#(record)}"#
        )
        ...
        let listed = try decodeEnvelope(
            #"{"version":27,"type":"session_files","request_id":"list-1","session_id":"chat-1","files":[{"origin":"user","file":\#(record)},{"origin":"agent","file":\#(record)}]}"#
        )
        ...
        let chunk = try decodeEnvelope(
            #"{"version":27,"type":"session_file_chunk","request_id":"read-1","session_id":"chat-1","file_id":"file-1","offset":0,"data":"AQID","next_offset":null}"#
        )
    ...
    func testFrontendBlockRequiresFilesInV28() {
        let fixture =
            #"{"id":null,"group":null,"update":"replace","state":"complete","role":"notice","title":"Done","text":"","symbol":null,"format":"plain_text","tone":"neutral"}"#
    ...
    func testWorkspaceViewerResponsesMatchV28() throws {
        ...
        let complete = try decodeEnvelope(
            #"{"version":27,"type":"workspace_files","request_id":"files-2","session_id":"chat-1","files":[{"path":"Sources/App.swift","size":3}]}"#
        )
        ...
        let chunk = try decodeEnvelope(
            #"{"version":27,"type":"workspace_file_chunk","request_id":"read-1","session_id":"chat-1","path":"Sources/App.swift","offset":0,"data":"AQID","next_offset":null}"#
        )
        guard case .workspaceFileChunk(_, _, let path, let offset, let data, let nextOffset) = chunk
        else {
    ...
    func testBotAndRoutineResponsesDecodeProtocolFields() throws {
        ...
        guard case .gitDiff(let diffRequestID, let diffSessionID, let scope, let diff) = gitDiff
        else {
        ...
        let routines = try decodeEnvelope(
            #"{"version":56,"type":"routines","request_id":"routine-1","routines":[{"id":"routine-1","bot_id":"bot-1","workspace":"/srv/mobius","instructions":"Review open pull requests","schedule":{"kind":"interval","at":null,"every_seconds":120,"expression":null,"time_zone":null},"ends_at":null,"enabled":true,"finished":false,"next_run_at":500}]}"#
        )
        ...
        let history = try decodeEnvelope(
            #"{"version":56,"type":"routine_history","request_id":"history-1","runs":[{"id":"run-1","routine_id":"routine-1","bot_id":"bot-1","started_at":100,"finished_at":110,"status":"succeeded","session_id":"chat-2","message":null}]}"#
        )
    ...
    func testControlProviderProfileAndDirectoryResponsesDecode() throws {
        guard case .authenticated = try decodeEnvelope(#"{"version":27,"type":"authenticated"}"#)
        else {
            return XCTFail("Expected authenticated envelope")
        }
        guard
            case .accepted(let acceptedID) = try decodeEnvelope(
                #"{"version":27,"type":"accepted","request_id":"request-1"}"#)
        else {
        ...
        guard
            case .providerCredentialSaved(let credentialID, let instance, let provider) =
                try decodeEnvelope(
                    #"{"version":27,"type":"provider_credential_saved","request_id":"credential-1","instance":"openai-work","provider":"openai_socket"}"#
                )
        else {
        ...
        guard
            case .pairingCode(let pairingID, let code, let expiresAt) = try decodeEnvelope(
                #"{"version":27,"type":"pairing_code","request_id":"pairing-1","code":"123456","expires_at":500}"#
            )
        else {
        ...
        guard
            case .providerLoginStarted(
                let loginRequestID, let loginID, let loginProvider, let verificationURL,
                let userCode) = try decodeEnvelope(
                    #"{"version":27,"type":"provider_login_started","request_id":"login-1","login_id":"device-1","provider":"openai_codex","verification_url":"https://example.com/device","user_code":"ABCD"}"#
                )
        else {
        ...
        guard
            case .providerLoginFinished(
                let finishedRequestID, let finishedLoginID, let finishedProvider) =
                try decodeEnvelope(
                    #"{"version":27,"type":"provider_login_finished","request_id":"login-1","login_id":"device-1","provider":"openai_codex"}"#
                )
        else {
        ...
        guard
            case .gitCredentialStatus(let gitCredentialID, let available, let legacyUsername) =
                try decodeEnvelope(
                    #"{"version":42,"type":"git_credential_status","request_id":"git-credential-1","available":true}"#
                )
        else {
        ...
        guard
            case .gitCredentialStatus(_, _, let username) = try decodeEnvelope(
                #"{"version":44,"type":"git_credential_status","request_id":"git-credential-2","available":true,"username":"octo"}"#
            )
        else {
        ...
        guard
            case .sshIdentities(let sshListID, let identities) = try decodeEnvelope(
                #"{"version":44,"type":"ssh_identities","request_id":"ssh-list-1","identities":[{"label":"id_ed25519","algorithm":"ssh-ed25519","fingerprint":"SHA256:safe"}]}"#
            )
        else {
        ...
        guard
            case .sshIdentityGenerated(
                ...
                #"{"version":44,"type":"ssh_identity_generated","request_id":"ssh-generate-1","identity":{"label":"id_ed25519","algorithm":"ssh-ed25519","fingerprint":"SHA256:new"},"public_key":"ssh-ed25519 AAAA mobius"}"#
            )
        else {
        ...
        guard
            case .directories(let directoryID, let listing) = try decodeEnvelope(
                #"{"version":27,"type":"directories","request_id":"directories-1","listing":{"path":"/srv","parent":null,"entries":[]}}"#
            )
        else {
    ...
    func testPairedRejectedAndErrorResponsesDecode() throws {
        guard
            case .paired(let clientID, let token) = try decodeEnvelope(
                #"{"version":27,"type":"paired","client_id":"phone-7","token":"bearer"}"#)
        else {
        ...
        guard
            case .rejected(let rejection) = try decodeEnvelope(
                #"{"version":27,"type":"rejected","request_id":"request-1","code":"conflict","message":"stale","fatal":false}"#
            )
        else {
        ...
        guard
            case .error(let failure) = try decodeEnvelope(
                #"{"version":27,"type":"error","code":"internal","message":"failed","fatal":true}"#)
        else {
    ...
    func testUnknownGatewayMessageAndOperationAreRejected() {
        XCTAssertThrowsError(try decodeEnvelope(#"{"version":27,"type":"future_message"}"#)) {
            error in
            XCTAssertEqual(
                error as? GatewayWireError,
        ...
        XCTAssertThrowsError(try decoder().decode(AgentOperation.self, from: Data(operation.utf8)))
        { error in
            XCTAssertEqual(
                error as? GatewayWireError,
