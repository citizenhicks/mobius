...
extension AppModelTests {
    ...
    func testReassigningChatUsesItsIDAndWaitsForConfirmedOwnership() async throws {
        ...
        XCTAssertEqual(app.chat.sessions.first?.sessionContext.ownerId, "bot-1")
        ...
        XCTAssertEqual(app.chat.sessions.first?.sessionContext.ownerId, "bot-2")
    ...
    func testTargetedHiddenBotSessionsStayOutsideChatsAndRemainSelectable() async throws {
        let recorder = GatewayRequestRecorder()
        let model = try model { request in await recorder.record(request) }
        let helper = bot()
        ...
        model.refreshBotSessions(helper.id)
        let request = await recorder.firstRequest(after: 0) { request in
    ...
    func testDeletingPresentedChatDoesNotReportRequestsInvalidatedByDeletion() async throws {
        let recorder = GatewayRequestRecorder()
        let model = try model { request in await recorder.record(request) }
        let selected = session(sessionID: "chat-1", state: .idle)
        model.gateway.connectionState = .ready
        ...
        model.destination = .chats
        model.navigationPath = [.chat(.session(selected.sessionId))]
        model.workspaceFilesRequestID = "workspace-files-1"

        model.deleteSession(selected)
        let request = await recorder.firstRequest(after: 0) { request in
            if case .deleteSessions = request { return true }
            return false
        }
        guard case .deleteSessions(let requestID, _) = try XCTUnwrap(request) else {
            return XCTFail("Expected a delete-session request")
        ...
        XCTAssertNil(model.toast)
        XCTAssertEqual(model.chat.pendingDeletedSessionIDs, [selected.sessionId])
        model.gateway.handle(.sessions(requestID: requestID, sessions: []))
        XCTAssertTrue(model.chat.pendingDeletedSessionIDs.isEmpty)
    }
    ...
    func testPeerMessagesAppearInTheNormalTranscript() throws {
        let app = try model { _ in }
        app.bots = [bot()]
        app.chat.sessionRequestID = "open"
        app.gateway.handle(
            .sessionOpened(
                requestID: "open",
                payload: sessionReady(latestSequence: 100)))
        app.chat.reduce(
            record: recorded(
                92,
                testMessageEvent(
        ...
        XCTAssertEqual(app.chat.transcript.last?.kind, .assistant)
        XCTAssertEqual(
